...
export default function Buy() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setFeedback({ type: 'error', message: 'Showing default catalog while we reconnect.' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleAddToCart = (product) => {
    ...
    if (stockCount - alreadyReserved <= 0) {
      setFeedback({ type: 'error', message: 'This pair is currently out of stock.' });
      return;
    }
    ...
    if (!result.success) {
      ...
      if (result.reason === 'NOT_AUTHENTICATED') {
        ...
        message = 'This pair is no longer available.';
      }
  ...
  return (
    <div className={styles.container}>
      ...
      {loading && (
        <div className={styles.feedback}>Loading catalog...</div>
      )}
        ...
        {products.map((product) => {
          ...
          <div key={product.id} className={styles.card}>
            <div className={styles.imageContainer}>
              ...
              />
              <span className={`${styles.badge} ${category === 'winter' ? styles.badgeWinter : styles.badgeSneaker}`}>
                {category}
              </span>
            </div>
