...
export default function Winter() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setFeedback({ type: 'error', message: 'Showing default winter boots while offline.' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const winterProducts = products.filter(
    (product) => normalizeCategory(product.category) === 'winter'
  );
  ...
  const handleAddToCart = (product) => {
    ...
    if (stockCount - alreadyReserved <= 0) {
      setFeedback({ type: 'error', message: 'This boot is currently out of stock.' });
      return;
    }
    ...
    if (!result.success) {
      ...
      if (result.reason === 'NOT_AUTHENTICATED') {
        ...
        message = 'This boot is no longer available.';
      }
  ...
  return (
    <div className={styles.container}>
      ...
      {loading && (
        <div className={styles.feedback}>Loading winter lineup...</div>
      )}

      <div className={styles.grid}>
        {!loading && winterProducts.length === 0 && (
          <div className={styles.feedback}>No winter boots yet. Add some from the worker panel.</div>
        )}
